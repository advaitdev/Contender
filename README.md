...
The arena world is created or loaded during plugin startup, and complete maps begin preparing automatically. Each copy becomes available as soon as it finishes; it does not wait for the whole pool. Starting a duel never creates a world or copies the source map. Between duel rounds, contestants and spectators briefly wait at the lobby while their arena is restored from the saved schematic, including air and entities. Their kits stay intact during this wait, and they return to the arena before the next countdown. Capture and reset load the selected chunks and their entity data before working, and keep them loaded until finished. The next round waits for that reset. An arena only becomes available for another duel after cleanup finishes.
