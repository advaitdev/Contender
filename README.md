...
The editor shows all eight settings as sliders on one screen. Close or Escape applies and saves the current values automatically. Vanilla dialogs submit inputs when an action runs, so dragging a slider does not change gameplay until the menu closes. The menu shows whether hacks are currently active. After closing it, a private message confirms whether your changes applied immediately or were saved for when you start playing. Each slider shows a warning threshold in brackets; higher values may look blatant. There is no separate Save or Review screen.
