...
**Pause Tournament** lets current matches finish. If a match stops unexpectedly, the tournament pauses automatically and shows the reason in `/tournament`. The pairing stays waiting until you resume or award its result; it does not restart on its own. Arena reset and player-return failures hold the affected duel and retry without triggering this tournament pause. Decided wins and forfeits are saved before arena cleanup; failed saves remain queued for another attempt. **Cancel Tournament** ends its active duels and stops its schedule. Tab immediately returns to the normal player list while the arenas finish resetting. Starting a community vote pauses the tournament and cancels active duels; cancelled matches remain queued for a later resume.
...
The action bar shows `Starts in 10`, then counts down until combat begins. Later rounds show a three-second countdown after the arena resets. The countdown clears when combat starts or the duel is cancelled. Duels have no boss bars or persistent score/action-bar messages. Duel setup errors appear in chat. Startup and round preparation verify each contestant reached their spawn before enabling combat. Blocked teleports keep the duel reserved and retry; a round is counted only after preparation succeeds. Arena worlds disable the locator bar and advancement chat announcements, and each duel reapplies these rules when it starts.
